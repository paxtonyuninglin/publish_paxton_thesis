...
temp_vals_day <- seq(
  min(AF_daily_table_scaled$avg_max_temp_scaled, na.rm = TRUE),
  max(AF_daily_table_scaled$avg_max_temp_scaled, na.rm = TRUE),
  length.out = 10
)

raw_temp_vals_day <- seq(
  min(AF_daily_table_scaled$avg_max_temp, na.rm = TRUE),
  max(AF_daily_table_scaled$avg_max_temp, na.rm = TRUE),
  length.out = 10
)
...
# compute quantiles
temp_quants_day <- quantile(AF_daily_table_scaled$temp_diff_scaled,
                            probs = c(0.25, 0.50, 0.75),
                            na.rm = TRUE)
temp_diff_quants_raw_day <- quantile(AF_daily_table_scaled$temp_diff,
                                     probs = c(0.25, 0.50, 0.75),
                                     na.rm = TRUE)
...
# build the grid
prediction_temp_diff_af_day <- expand.grid(
  temp_diff_scaled = as.numeric(temp_quants_day),
...
prediction_temp_diff_af_day <- prediction_temp_diff_af_day %>%
  left_join(temp_quants_day_joined, by = "temp_diff_scaled")


# convert back to raw temperatures
temp_mean_day <- mean(AF_daily_table_scaled$avg_max_temp, na.rm = TRUE)
temp_sd_day   <- sd(AF_daily_table_scaled$avg_max_temp, na.rm = TRUE)
...
deer_pred     <- predict_from_model(af_deer_temp_diff,     prediction_temp_diff_af_day)
bear_pred     <- predict_from_model(af_bear_temp_diff,     prediction_temp_diff_af_day)
squirrel_pred <- predict_from_model(af_squirrel_temp_diff,     prediction_temp_diff_af_day)

# Add species labels
deer_pred$Species <- "Mule deer"
bear_pred$Species <- "Black bear"
squirrel_pred$Species <- "Red squirrel"
...
levs_day <- levels(prediction_temp_diff_af_day$temp_diff_rounded)
...
af_temp_pred <-ggplot(all_pred ,aes(x = avg_max_temp,y = pred, color = temp_diff_rounded,
...
af_temp_pred <- af_temp_pred +
  force_panelsizes(rows = unit(6, "cm"), cols = unit(9, "cm"))

combined <- plot_grid(
  af_temp_pred, tl_plot,
  ncol = 2,
  align = "hv",       # aligns panels horizontally and vertically
  axis = "tblr"        # aligns top/bottom/left/right axes across both
) +
